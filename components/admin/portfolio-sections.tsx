...
function ProjEntry({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="proj-card">
      {data.image
        ? <img src={str(data.image)} alt={str(data.title)} style={{width:"100%",aspectRatio:"16/9",objectFit:"cover",borderRadius:"8px",marginBottom:"0.5rem"}} />
      ...
      <div style={{display:"flex",gap:"0.5rem",marginTop:"0.25rem",flexWrap:"wrap"}}>
        {data.github && <a href={str(data.github)} target="_blank" rel="noopener" className="entry-link">GitHub ↗</a>}
        {data.demo && <a href={str(data.demo)} target="_blank" rel="noopener" className="entry-link">Demo ↗</a>}
        {data.paper && <a href={str(data.paper)} target="_blank" rel="noopener" className="entry-link">Paper ↗</a>}
      </div>
...
function SkillsEntry({ entries }: { entries: Entry[] }) {
  ...
  const programming = all.find(d => d.programming) as { programming?: { name: string; level: number; icon?: string }[]; frameworks?: { name: string; level: number }[]; hardware?: string[]; software_tools?: { name: string; category: string }[]; soft_skills?: string[]; languages?: { name: string; level: string }[] } | undefined;
    ...
    <div className="skills-grid">
      ...
      {d.hardware && (
        <div className="skill-card">
          ...
          <div className="chip-cloud">{d.hardware.map((h) => <span key={h} className="chip">{h}</span>)}</div>
        </div>
      )}
      {d.software_tools && (
        <div className="skill-card">
          <div className="skill-card-title">Software & Tools</div>
          <div className="chip-cloud">{d.software_tools.map((t) => <span key={t.name} className="chip">{t.name}</span>)}</div>
        </div>
      )}
      {d.soft_skills && (
        <div className="skill-card">
          <div className="skill-card-title">Soft Skills</div>
          <div className="chip-cloud">{d.soft_skills.map((s) => <span key={s} className="chip">{s}</span>)}</div>
        </div>
      )}
      {d.languages && (
        <div className="skill-card">
        ...
        <section key={s.id} id={s.slug} className="block" style={i % 2 === 0 ? {} : {background:"var(--bg2)"}}>
          <div className="container">
            ...
            {s.entries.length > 0 ? renderSection(s) : (
              <p style={{color:"var(--muted2)",fontSize:"0.875rem"}}>No entries yet — add them from the admin panel.</p>
            )}
          </div>
        </section>
      ))}
